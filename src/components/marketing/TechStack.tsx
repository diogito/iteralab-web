...
function TypeScriptIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M3 3h18v18H3V3zm10.71 14.86c.5.98 1.51 1.73 3.09 1.73 1.6 0 2.8-.83 2.8-2.36 0-1.41-.81-2.04-2.25-2.66l-.42-.18c-.73-.31-1.04-.52-1.04-1.02 0-.41.31-.73.81-.73.48 0 .8.21 1.09.73l1.31-.87c-.55-.96-1.33-1.33-2.4-1.33-1.51 0-2.48.96-2.48 2.23 0 1.38.81 2.03 2.03 2.55l.42.18c.78.34 1.24.55 1.24 1.13 0 .48-.45.83-1.15.83-.83 0-1.31-.43-1.67-1.03l-1.38.8zM10 17v-6h2.07v-1.46H5.26V11H7.5v6H10z" />
    </svg>
  );
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection>
          <p className="text-center text-xs uppercase tracking-widest text-zinc-500 mb-8">
            Potenciado por arquitectura moderna
          </p>
          <div className="flex justify-center items-center gap-8 md:gap-16">
            {technologies.map((tech) => (
              ...
              >
                <tech.icon className="w-8 h-8 text-zinc-700 group-hover:text-white transition-colors duration-300" />
                <span className="text-xs text-zinc-600 group-hover:text-zinc-400 transition-colors duration-300 font-display font-medium">
                  {tech.name}
                </span>
              </div>
            ))}
